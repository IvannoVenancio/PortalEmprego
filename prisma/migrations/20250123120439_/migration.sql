/*
  ...
  - You are about to drop the column `updated_at` on the `user_type` table. All the data in the column will be lost.
  ...
  - Added the required column `update_at` to the `user_type` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `user_type` DROP COLUMN `updated_at`,
    ADD COLUMN `update_at` DATETIME(3) NOT NULL,
    MODIFY `created_at` TIMESTAMP(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6);
...
CREATE TABLE `Vaga` (
    ...
    `id_requisitos_qualificacoes_fk` INTEGER NULL,
    `tipo_emprego` VARCHAR(191) NOT NULL,
    `id_idiomas_fk` INTEGER NOT NULL,
    `data_criacao` DATETIME(3) NOT NULL,
    `latitude` DOUBLE NULL,
    `longitude` DOUBLE NULL,
    `faixa_salarial` VARCHAR(191) NULL,
...
-- AddForeignKey
ALTER TABLE `Candidato` ADD CONSTRAINT `Candidato_id_idiomas_fk_fkey` FOREIGN KEY (`id_idiomas_fk`) REFERENCES `Idioma`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `Vaga` ADD CONSTRAINT `Vaga_id_requisitos_qualificacoes_fk_fkey` FOREIGN KEY (`id_requisitos_qualificacoes_fk`) REFERENCES `RequisitosQualificacoes`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `Vaga` ADD CONSTRAINT `Vaga_id_idiomas_fk_fkey` FOREIGN KEY (`id_idiomas_fk`) REFERENCES `Idioma`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
